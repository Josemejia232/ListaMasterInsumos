...
class PromartScraper(BaseScraper):
    def detect(self) -> bool:
        return "promart.com" in self.url.lower()
    ...
    def scrape(self) -> ProductoScraped:
        ...
        codigo = ""
        match = re.search(r'"sku"\s*:\s*["\']?(\d+)["\']?', resp.text)
        if match:
            codigo = match.group(1)
        if not codigo:
            sku_el = soup.find("span", class_=re.compile(r"sku|codigo", re.I))
            if sku_el:
        ...
        unidad = "Unidad"
        unit_el = soup.find("span", class_=re.compile(r"unit|medida", re.I))
        if unit_el:
